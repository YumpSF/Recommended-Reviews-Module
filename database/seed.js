...
for (let i = 0; i < 100; i++) {
  ...
  const queryString = `Insert INTO user_info(user_name, location, number_reviews, number_photos) VALUES(?, ?, ?, ?)`;
  db.connection.query(queryString, [obj.user_name, obj.location, obj.number_reviews, obj.number_photos], (err) => {
    if (err) {
      console.log(err);
    }
  });
